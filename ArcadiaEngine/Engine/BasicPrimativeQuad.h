...
public:
	BasicPrimativeQuad(float width = 10.0f, float height = 10.0f, bool showLines = true)
	{
		SetWidth(width);
		SetHeight(height);
	...
	inline void SetValues(float width = 10.0f, float height = 10.0f, bool showLines = true, float rotationSpeed = 0.0f, GLuint textureID = 0, tdogl::Program* shaderProgram = nullptr) {
		SetWidth(width);
		SetHeight(height);
